  ...
  lazy val defaultSettings = resolverSettings ++
    ...
    Protobuf.settings ++ Seq[Setting[_]](
      ...
      scalacOptions in Compile ++= (
        if (true) Seq("-target:jvm-1.8")
        else if (scalaBinaryVersion.value == "2.11" || System.getProperty("java.version").startsWith("1."))
          Seq("-target:jvm-1.8", "-javabootclasspath", CrossJava.Keys.fullJavaHomes.value("1.8") + "/jre/lib/rt.jar")
        else
          // -release 8 is not enough, for some reason we need the 8 rt.jar explicitly #25330
          Seq("-release", "8", "-javabootclasspath", CrossJava.Keys.fullJavaHomes.value("1.8") + "/jre/lib/rt.jar")),
      scalacOptions in Compile ++= (if (allWarnings) Seq("-deprecation") else Nil),
      scalacOptions in Test := (scalacOptions in Test).value.filterNot(opt ⇒
        opt == "-Xlog-reflective-calls" || opt.contains("genjavadoc")),
      javacOptions in compile ++= DefaultJavacOptions ++ Seq("-source", "9","-target", "9"/*, "-bootclasspath", CrossJava.Keys.fullJavaHomes.value("1.8") + "/jre/lib/rt.jar"*/),
      javacOptions in test ++= DefaultJavacOptions ++ Seq("-source", "9", "-target", "9" /*, "-bootclasspath", CrossJava.Keys.fullJavaHomes.value("1.8") + "/jre/lib/rt.jar"*/),
